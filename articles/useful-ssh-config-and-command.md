...
- `-t` オプション
  - 暗号化方式を指定できます。
  - オプションがない場合、一般的な RSA 方式の鍵が作成されます。
- `-f` オプション
  - 鍵のファイルパスを指定できます。
  - オプションがない場合、入力を求められます。
- `-b` オプション
  - 暗号化強度（鍵の長さ）を指定できます。[^2]
  - オプションがない場合、`3072bit` になります。（RSA 方式の場合です。）
- `-N` オプション
  - パスフレーズを指定できます。
  - オプションがない場合、入力を求められます。
- `-C` オプション
...
- ssh-agent から登録を解除する方法です。
  - 実際には、めったに実行しません。
    - ログアウトなど、セッションが終了すると自動的に ssh-agent は停止するためです。
...
- `-l $USER` もしくは `$USER@` をログインサーバの頭につけることで指定できます。
...
- リモートにあるサーバーのポートを、ローカルのクライアントにフォワーディングすることで、クライアントのネットワークからリモートのサーバーのポートにアクセスできます。[^3]
...
記載していたパラメーターの詳細は以下の通りです。使用頻度が高かったものをまとめました。

- Host: ホスト名
  - 記載したホスト名で SSH できるようになります。
  - ワイルドカードで指定もできます。
    - `*` と記載することで、すべてのサーバーに対して共通の設定ができます。
    - スペース区切りで複数指定できます
- HostName:
  - ホスト名、もしくは IP アドレスを記載します。
- IdentityFile
  - 秘密鍵のパスを記載します。
  - コマンドオプションは `-i`
- User
...
- Port
  ...
  - コマンドオプションは `-p`
- ForwardAgent
  - エージェントフォワードの設定を記載します。
  - コマンドオプションは `-A`
- TCPKeepAlive
  - サーバーとの接続を保持するかどうかの設定を記載します。
- ServerAliveInterval
  - サーバーからの応答がない場合、接続を閉じる時間を記載します。
- StrictHostKeyChecking
  - ログインしたサーバーを `~/.ssh/known_hosts` に追加するか否かの設定です。
- UserKnownHostsFile
  - `known_hosts` ファイルパスを記載します。
- ProxyCommand
  - ポートフォワードする時などに使用します。
...
- -L と -R オプションの違いが図解されており、とてもわかりやすかったです。[^3]
...
[^3]: リモートホストにポートフォワーディングする `-R` オプションもありますが、実際に使ったことはなかったので割愛します。
